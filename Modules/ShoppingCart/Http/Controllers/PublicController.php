...
use Modules\Wallet\Events\MinusBalanceWallet;
...
use Modules\ShoppingCart\Emails\OrderComplete;
...
class PublicController extends BasePublicController
{
    ...
    public function quickBuy(Request $request)
    {
        ...
        if ($product) {
            ...
            $price = $product->price;
            if ($authCheck && $product->price_member > 0 && $discount > 0) {
                $price = $product->price_member * (1 - $discount / 100);
            } elseif ($authCheck && $product->price_member > 0 && $discount == 0) {
                $price = $product->price_member;
            }
            Cart::add([
                'id' => $productId, 'name' => $product->title, 'qty' => $quantity, 'price' => $price,
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($product) {
            ...
            if($avatar != '')
            {
                $avatarPath = $avatar->path_string;
            }
            if ($product->is_promotion == 1) {
                $price = $product->price_sale;
            }
            elseif ($authCheck && $product->price_member > 0 && $product->price_member < $product->price_sale && $product->is_promotion == 1) {
                $price = $product->price_sale;
            }
            elseif ($authCheck && $product->price_member > 0) {
                $price = $product->price_member;
            }
            else {
                $price = $product->price;
            ...
            Cart::add([
                'id' => $productId, 'name' => $product->title, 'qty' => $quantity, 'price' => $price,
                'options' => ['avatar' =>  $avatarPath, 'slug' => $product->slug, 'code' => $product->code, 'price_old' => $product->price_sale]
            ]);
    ...
    public function updateQty(Request $request)
    {
        try {
            ...
            return response()->json([
                'error' => false,
                'total' => number_format($total), 'subtotal' => number_format($subtotal), 'count' => Cart::count(), 'rowTotal' => number_format($rowTotal)
            ]);
        } catch (\Exception $e) {
            return response()->json(['error' => true, 'message' => $e->getMessage()]);
        }
    ...
    public function getCart()
    {
        ...
        $total = formatNumber($subtotal);
        $carts = Cart::content();
        return view('shoppingCarts.cart', ['carts' => $carts,'total' => number_format($total), 'subtotal' => number_format($subtotal)]);
    }
    ...
    public function getOrderDetail($code)
    {
        $order = $this->orderRepository->findByAttributes(['order_code' => $code]);
        if ($order) {
            $order_status = trans(StatusOrderEnum::getValue($order->status));
            $orderDetails = $order->orderDetails;
            return view('shoppingCarts.order-detail', compact('order', 'order_status', 'orderDetails'));
        } else {
            return redirect()->route('homepage');
        }
    }
    ...
    public function getCheckout()
    {
        ...
        if ($count == 0) {
            ...
            $total = formatNumber($subtotal);
            $carts = Cart::content();
            return view('shoppingCarts.checkout', ['carts' => $carts, 'total' => number_format($total), 'subtotal' => number_format($subtotal)]);
        }
    }
    ...
    public function checkout(Request $request)
    {
        try {
            ...
            if ($count > 0 &&  $subtotal > 0) {
                ...
                $totalPay = $subtotal;
                $product_detail_ref_code_aff = session('product_detail_ref_code_aff');

                $ref_code = isset($product_detail_ref_code_aff) ? $product_detail_ref_code_aff[0] : null;
                ...
                $order = [
                    ...
                    'note' => null,
                    'total' => $totalPay,
                    'total_item' => $count = Cart::count(),
                    'time_ship' => null,
                    'payment_method' => $request->payment_method,
                    'delivery_method' => $request->delivery_method,
                    'ref_code' => $ref_code,
                    'status' => StatusOrderEnum::getName('CREATED'),
                ...
                if (($request->payment_method == 3 || $request->payment_method == 2) && $totalPay > 0) {
                    return $this->hanldeCheckoutAlepay($order);
                } else {
                    $email = new OrderComplete($order);
                    Cart::destroy();
    ...
    public function myPurchase(Request $request)
    {
        try {
            ...
            $orders = $this->orderRepository->paginateForCustomer($request, $customerID);

            $statuses = ['CREATED', 'SHIPPING', 'PAYMENTING', 'PAYMENT_COMPLETED', 'PAYMENT_FAILED', 'PROCESSING', 'COMPLETED', 'CANCELED'];

            return view('shoppingCarts.my-purchase', compact('orders', 'statuses'));
        } catch (\Exception $e) {
            \Log::error($e->getMessage());
            return back();
        }
    ...
    public function myPurchaseDetail($orderId, Request $request)
    {
        try {
            $customerID = auth()->guard('customer')->user()->id;
            $orders = $this->orderRepository->find($orderId);
            $orderDetails = $this->orderDetailRepository->getByAttributes([
                'order_id' => $orderId,
            ]);
            $products = $this->productRepository->all();            
            return view('shoppingCarts.my-purchase-detail', compact('orders', 'orderDetails', 'products'));
        } catch (\Exception $e) {
            \Log::error($e->getMessage());
            return back();
        }
    ...
    private function hanldeCheckoutAlepay($order)
    {
        try {
            ...
            if ($data !== false && isset($data->code) && $data->code == "000") {
                $this->orderRepository->update($order, ['payment_code' => $data->transactionCode, 'status' => StatusOrderEnum::getName('PAYMENTING')]);
                ...
                $this->orderRepository->update($order, ['payment_code' => $data->transactionCode, 'status' => StatusOrderEnum::getName('PAYMENT_FAILED')]);
    ...
    public function alepayCheckoutSuccess($order_code, Request $request)
    {
        if (isset($request->transactionCode) && isset($request->errorCode) && $request->errorCode == '000') {
            $order = $this->orderRepository->findByAttributes(['order_code' => $order_code, 'status' => StatusOrderEnum::getName('PAYMENTING'), 'payment_code' => $request->transactionCode]);
            if ($order) {
                Mail::to($order->email)->send(new OrderConfirm($order));
                $this->orderRepository->update($order, ['status' => StatusOrderEnum::getName('PAYMENT_COMPLETED')]);
                $email = new OrderComplete($order);
                Mail::to($request->email)->send($email);
    ...
    public function alepayCheckoutFail($order_code)
    {
        ...
        if ($order) {
            $this->orderRepository->update($order, ['status' => StatusOrderEnum::getName('PAYMENT_FAILED')]);
