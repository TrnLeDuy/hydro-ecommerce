...
class PublicController extends BasePublicController
{
    ...
    public function uri($slug)
    {
        ...
        $blogs = $this->page->getAllBlogs();
        $categories = $this->postCategory->getAllCategory();
        if (!session()->has('latestNews')) {
            $latestNews = $blogs->take(3);
            session()->put('latestNews', $latestNews);
        } else {
            $latestNews = session()->get('latestNews');
        }

        $this->addAlternateUrls($this->getAlternateMetaData($page));

        return view($template, compact('page', 'blogs', 'latestNews', 'categories'));
    }
    ...
    public function homepage()
    {
        ...
        return view($template, compact('page', 'latestNews'));
    }
